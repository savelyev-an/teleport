...
The Teleport user joining the session must have a `join_sessions` policy set up that allows
them to join sessions created by the other user. If `joe` has a role named `access`, the other
user needs have a role with a policy as shown below:
You can find more detailed information about `join_sessions` policies in the [role reference](../../access-controls/reference.mdx) and [Moderated Sessions guide](../../access-controls/guides/moderated-sessions.mdx).

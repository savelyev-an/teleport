...
`authentication.type` controls the authentication scheme used by Teleport. Possible values are `local` and `github` for OSS, plus `oidc`, `saml` for Enterprise.
...
When disabled, users can only login through non-local auth like `saml`, `oidc` or `github`.

[Disabling local auth is required for FedRAMP / FIPS](../../../enterprise/ssh-kubernetes-fedramp.mdx).

...
func Run(options Options) (app *kingpin.Application, executedCommand string, conf *service.Config) {
	...
	appStartCmd.Flag("proxy-server", "Address of the proxy server [none].").StringsVar(&ccf.ProxyServerAddr)
	...
	dbStartCmd.Flag("proxy-server", "Address of the proxy server [none].").StringsVar(&ccf.ProxyServerAddr)

...
type Server struct {
	...
	*Services
	...
	// Cache should either be the same as Services, or a caching layer on top of it
	Cache
...
func (a *Server) NewWebSession(ctx context.Context, req types.NewWebSessionRequest) (types.WebSession, error) {
	...
	roleSet, err := services.FetchRoles(req.Roles, a, req.Traits)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	clusterName, err := a.GetClusterName()
...
func (a *Server) DeleteNamespace(namespace string) error {
	...
	return a.DeleteNamespace(namespace)
}
...
func (a *Server) addAddtionalTrustedKeysAtomic(
	...
	for {
		...
		currentCA, err = a.GetCertAuthority(ctx, currentCA.GetID(), true)
		if err != nil {
			return trace.Wrap(err)
		}
	}

...
func TestRewritePaths(t *testing.T) {
	...
	defer re.Body.Close()
	require.Equal(t, re.StatusCode, http.StatusOK)
	require.Equal(t, handler.capturedNamespace, "default")
	require.Equal(t, handler.capturedID, "s1")

	re, err = http.Post(server.URL+"/v1/namespaces/system/sessions/s2/stream", "text/json", nil)
	require.NoError(t, err)
	defer re.Body.Close()
	require.Equal(t, re.StatusCode, http.StatusOK)
	require.Equal(t, handler.capturedNamespace, "system")
	require.Equal(t, handler.capturedID, "s2")
}
